...
class OrderPage(BasePage):
    ...
    def send_keys_date_by_keyboard_input(self):
        self.send_keys_to_input(OrderPageLocators.date).send_keys(TestData.test_data_user_1[5])

    @allure.step("Кликнуть по выбранной дате в выпадающем календаре поля ввода даты начала аренды")
    def click_date_in_calender(self):
        self.click_on_element(OrderPageLocators.calender_item)
